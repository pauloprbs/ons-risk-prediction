...
cmo_diario as (
    ...
    select
        ...
        last_value(c.cmo_medio_semanal ignore nulls) over (order by d.date_day) as cmo_semanal_seco
    from {{ ref('int_all_days') }} d
    left join cmo_semanal c on d.date_day = c.dia_semana
)
...
from {{ ref('int_all_days') }} d
left join disponibilidade_diaria disp on d.date_day = disp.dia
left join cmo_diario cmo on d.date_day = cmo.dia